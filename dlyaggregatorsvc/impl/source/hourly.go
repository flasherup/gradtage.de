...
func (h Hourly) FetchTemperature(ch chan *parser.StationDaily, ids []string) {
	huorlyUpdates := h.hrlService.GetUpdateDate(ids)
	dailyUpdates := h.dlyService.GetUpdateDate(ids)
...
func (h Hourly)fetchStation(id string, ch chan *parser.StationDaily, start string, end string) {
	period := h.hrlService.GetPeriod(id, start, end)
	if period.Err != "nil" {
		level.Error(h.logger).Log("msg", "GetPeriod hourly error", "err", period.Err)
		ch <- nil
		return
	}
...
func (h Hourly)hourlyToDaily(src []*grpc.Temperature) []dailysvc.Temperature {
