...
func NewTemperatureChangeAlert(prev hrlgrpc.Temperature, cur hourlysvc.Temperature) alertsvc.Alert {
	...
	return alertsvc.Alert{
		Name: "Plausibility",
		Desc: "Hourly temperature change more than 5°",
		Params: map[string]string{ "Previous":p,  "Current":c },
	}
}

func NewTemperatureGapAlert(prev hrlgrpc.Temperature, cur hourlysvc.Temperature) alertsvc.Alert {
	p := fmt.Sprintf("date: %s, temp:%g:", prev.Date, prev.Temperature)
	c := fmt.Sprintf("date: %s, temp:%g:",cur.Date, cur.Temperature)
	return alertsvc.Alert{
		Name: "Plausibility",
